...
// Mükerer kaydı engellemek için basit bir kontrol değişkeni
let sonKurulumTarihi = ""; 

const server = http.createServer(async (req, res) => {
    if (req.url === '/vakitleri-kur') {
        try {
            // --- MÜKERER EMİR KONTROL SATIRI ---
            const bugun = new Date().toLocaleDateString("tr-TR", {timeZone: "Europe/Istanbul"});
            if (sonKurulumTarihi === bugun) {
                console.log("Zaten cevap verdim, artık gerek kalmadı.");
                res.writeHead(200, { 'Content-Type': 'text/plain; charset=utf-8' });
                return res.end("Zaten kuruldu, mükerrer işlem engellendi.");
            }
            ...
            // İşlem başarıyla biterse tarihi kaydet ki bir sonraki dürtmede çalışmasın
